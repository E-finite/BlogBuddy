...
def ingest_website(
    ...
    if len(crawled_pages) == 0:
        logger.warning(
            f"No pages crawled from {base_url}. Check if the site is accessible and doesn't block crawlers.")
        return {
            "site_id": site_id,
            "pages_crawled": 0,
            "pages_stored": 0,
    ...
    if len(crawled_pages) > 0 and pages_stored == 0:
        logger.warning(
            "⚠️ No content extracted - this might be a JavaScript-rendered site (SPA/React/Vue)")
        logger.warning(
            "💡 These sites need a headless browser or manual content input")
