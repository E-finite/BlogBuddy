        ...
        try:
            ...
            if response.status_code in [429, 500, 502, 503, 504]:
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"Request failed with {response.status_code}, retrying in {delay}s (attempt {attempt + 1}/{max_retries})")
                    time.sleep(delay)
                    continue
            return response
...
def create_post(site: Dict[str, Any], payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        f"Creating WordPress post with payload keys: {list(payload.keys())}")

    try:
        response = _retry_request(lambda: requests.post(
            url, json=payload, auth=auth, timeout=30))

        if not response.ok:
            error_body = response.text
            logger.error(
                f"WordPress API error {response.status_code}: {error_body}")
...
def set_yoast_meta(site: Dict[str, Any], post_id: int, focuskw: str, seo_title: str, meta_desc: str) -> Dict[str, Any]:
    ...
    if response.status_code == 404:
        logger.warning(
            f"Yoast API endpoint not found (plugin may be inactive): {url}")
...
def link_polylang_translations(site: Dict[str, Any], translations_map: Dict[str, int]) -> Dict[str, Any]:
    ...
    if response.status_code == 404:
        logger.warning(
            f"Polylang API endpoint not found (plugin may be inactive): {url}")
        return {"status": "skipped", "reason": "endpoint_not_found"}
    response.raise_for_status()
    return response.json()
