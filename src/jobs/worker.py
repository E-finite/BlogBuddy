        ...
        try:
            ...
            for job_data in queued_jobs:
                ...
                if not job or "payload" not in job:
                    ...
                    db.update_job(job_id, "failed", error={"error": "Job not found or missing payload"})
                    continue
                payload = job["payload"]
                ...
                try:
                    ...
                    db.update_job(job_id, "failed", error={"error": str(e), "type": type(e).__name__})
                    db.add_job_step(job_id, "complete", "failed", {"error": str(e)})
