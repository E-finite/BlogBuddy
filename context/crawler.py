...
class WebsiteCrawler:
    ...
    def _fetch_page(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            canonical_url = canonical_tag.get(
                "href") if canonical_tag else final_url
    ...
    def crawl(self, seed_urls: List[str]) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Max depth: {self.max_depth}, Max pages: {self.max_pages}")
        ...
        while self.to_visit and len(self.results) < self.max_pages:
            ...
            logger.info(
                f"Fetching [{len(self.results)+1}/{self.max_pages}] depth={depth}: {url}")
        ...
        logger.info(
            f"Crawl complete. Fetched {len(self.results)} unique pages.")
        return self.results
